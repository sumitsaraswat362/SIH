...
export async function POST(req: Request) {
  try {
    ...
    const mailOptions = {
      ...
      to: recipient,
      subject: subject || 'Annapurna: New Buyer Matched for Your Produce',
      text: body || '',
      html: html || `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #28a745, #20c997); padding: 20px; border-radius: 16px 16px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 24px;">🌱 New Buyer Match</h1>
          </div>
          <div style="background: #f8f9fa; padding: 20px; border-radius: 0 0 16px 16px; border: 1px solid #e9ecef;">
            <p style="font-size: 16px; color: #333; line-height: 1.6;">${body || 'Hello Farmer, a new buyer has been matched for your produce. We negotiated a price better than the local mandi rate! Login to see buyer details.'}</p>
            <hr style="border: none; border-top: 1px solid #e9ecef; margin: 16px 0;">
            <p style="font-size: 12px; color: #999;">Sent by Annapurna AI Agricultural Marketplace</p>
          </div>
        </div>
      `,
    };
