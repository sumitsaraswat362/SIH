...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !password.trim()) { setError("Name and password are required"); return; }
    ...
    const err = await login({
      ...
      phone: phone.trim(),
      location: loc,
      city: role === "farmer" ? district : city,
      address: role === "farmer" ? `${village}, ${district}` : address,
    ...
    <main className="min-h-dvh flex flex-col items-center justify-center p-6 relative overflow-hidden bg-[var(--bg-primary)] aura-container">
      ...
      >
        ...
        <div className="text-center mb-8 relative">
          ...
          <h1 className="text-4xl font-extrabold tracking-tight mb-1.5 text-[var(--text-primary)]">Annapurna</h1>
          <p className="text-[var(--text-secondary)] text-base font-semibold tracking-wide">Direct Farm-to-Fork Marketplace</p>
          <p className="text-[var(--text-tertiary)] text-xs mt-1">SIH 26033 • Ministry of Consumer Affairs</p>
        </div>

        {/* Form */}
        <form onSubmit={handleLogin} className="p-7 relative rounded-[36px] glass shadow-[0_20px_40px_rgba(0,0,0,0.05)] dark:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.8)]">
          ...
          <div className="space-y-4 relative z-10">
            ...
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)}
                placeholder={role === "farmer" ? "e.g. Ramesh Patil" : "e.g. Anita Sharma"} required
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" value={district} onChange={(e) => setDistrict(e.target.value)} placeholder="e.g. Nashik"
                  ...
                  <div>
                    ...
                    <input type="text" value={village} onChange={(e) => setVillage(e.target.value)} placeholder="e.g. Vinchur"
                      className="w-full bg-[var(--fill-secondary)] border border-[var(--separator)] rounded-[16px] px-4 py-3.5 text-[var(--text-primary)] placeholder:text-[var(--text-tertiary)] focus:outline-none focus:ring-2 focus:ring-[#34C759] focus:border-transparent transition-all text-sm font-semibold"
                    />
                  </div>
                </div>
                <button type="button" onClick={requestLocation}
                  className={`w-full flex items-center justify-center gap-2 py-3.5 rounded-[16px] text-sm font-bold transition-all border ${
                    geoStatus === "success" ? "bg-[#34C75915] border-[#34C759] text-[#34C759]" :
                    geoStatus === "error" ? "bg-[#FF3B3015] border-[#FF3B30] text-[#FF3B30]" :
                    ...
                    {(["consumer", "retailer", "restaurant", "bulk_buyer"] as const).map((bt) => (
                      <button key={bt} type="button" onClick={() => setBuyerType(bt)}
                        className={`py-2.5 px-3 rounded-[14px] text-xs font-bold transition-all border ${
                          buyerType === bt ? "bg-[#007AFF15] border-[#007AFF] text-[#007AFF]" : "bg-[var(--fill-secondary)] border-[var(--separator)] text-[var(--text-secondary)]"
                        }`}
                      >
            ...
            <motion.button type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className={`w-full py-4 rounded-[20px] font-bold text-base text-white shadow-lg transition-all ${
                role === "farmer" ? "bg-gradient-to-r from-[#34C759] to-[#30D158] shadow-[#34C75940]" : "bg-gradient-to-r from-[#007AFF] to-[#5AC8FA] shadow-[#007AFF40]"
              }`}
            >
            ...
            <div className="flex items-center justify-center gap-4 pt-2">
              <span className="text-[10px] text-[var(--text-tertiary)]">🔒 Secure</span>
              <span className="text-[10px] text-[var(--text-tertiary)]">🇮🇳 Made in India</span>
              <span className="text-[10px] text-[var(--text-tertiary)]">🏛️ SIH 26033</span>
            </div>
          </div>
        </form>

        <div className="mt-6 text-center">
          <p className="text-xs text-[var(--text-tertiary)]">Ministry of Consumer Affairs, Food & Public Distribution</p>
        </div>
      </motion.div>
      <ThemeToggle />
    </main>
